        ...
        <div className="w-full py-3 " >
            <div className="flex gap-3 items-center mb-1 ">
                ...
                <span className="text-ascent-2 text-sm">
                    {moment(reply?.creadtedA).fromNow()}
                </span>
                </div>
            </div>
            <div className="ml-12">
    ...
    <>
      ...
      >
        <div className="w-full flex items-center gap-2 py-4 ">
          ...
          <TextInput
            name="comment"
            styles="w-full rounded-full py-3 px-3"
            placeholder={replyAt ? `Reply @${replyAt}` : "Comment this post"}
            register={register("comment", {
              required: "Comment can not be empty",
            })}
            error={errors.comment ? errors.comment.messge : ""}
          />
          {errMsg?.message && (
            <span
              role="alert"
          ...
          <div className="flex items-center justify-end pb-2">
            {loading ? (
              ...
              <CustomButton
                type="Submit"
                title="Comment"
                containerStyles="bg-[#0444a4] text-white py-2 mt-3 px-3 rounded-full 
...
const PostCard = ({ post, user, deletePost, likePost }) => {
  ...
  return (
    <div className="mb-2 bg-primary p-4 rounded-xl ">
      ...
      <div
        className="mt-4 flex items-center justify-between px-3 py-2 text-ascent-2
                ...
                {/* REPLIES */}
                  ...
                  <div className="py-2 px-8 mt-6">
                    {comment?.replies?.length>0 && (
                        ...
                        onClick={()=>setShowReply(showReply===comment?.replies?._id ? 0:
                            comment?.replies?._id)}
                        >
                    ...
                    {
                        showReply === comment?.repiles?._id &&
                        comment?.replies?.map((reply)=>(<ReplyCard
                        reply = {reply}
                        user = {user}
                        key ={reply?._id}
